...
mod tests {
    ...
    async fn get_list_of_games_200() {
        ...
        let res = warp::test::request().method("GET").path("/shop").reply(&filter).await;
    ...
    async fn get_list_of_games_with_options_200() {
        ...
        let res = warp::test::request()
            .method("GET")
            .path("/transaction?offset=1&limit=5")
            .reply(&filter)
            .await;
    ...
    async fn get_empty_list_with_offset_overshot_200() {
        ...
        let res = warp::test::request()
            .method("GET")
            .path("/transaction?offset=5&limit=5")
            .reply(&filter)
            .await;
